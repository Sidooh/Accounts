...
func (api *ApiClient) ensureAuthenticated() {
	err := api.authenticate(utils.MyCustomClaims{Email: "aa@a.a"})
	if err != nil {
		logger.ClientLog.Printf("error authenticating: %v", err)
	}
}

func (api *ApiClient) authenticate(data utils.MyCustomClaims) error {
	validity := time.Duration(viper.GetInt("ACCESS_TOKEN_VALIDITY")) * time.Minute * 2

	token, err := utils.GenerateToken(data, validity)
	if err != nil {
		return err
	}

	if api.cache != nil {
		api.cache.Set("token", token, 14*time.Minute)
	}
